...
export const calculateTravelTimeMins = (
	p1: Feature<Point>,
	p2: Feature<Point>,
): number => {
...
export const generateItinerary = async (
	...
	while (currentMins < endOfDayMins) {
		...
		const fixedBlock = blocks.find(
			(b) => b.type === 'fixed' && timeToMins(b.startTime) === currentMins,
		);
		if (fixedBlock) {
		...
		for (const poi of candidatePOIs) {
			...
			const travelMins = currentLocation
				? calculateTravelTimeMins(currentLocation, poi)
				: 0;
			const totalRequiredMins =
				travelMins + poi.properties.typical_duration_minutes;
			...
			if (totalRequiredMins <= availableMins) {
				...
				const departureMins =
					arrivalMins + poi.properties.typical_duration_minutes;
		...
		if (bestPoi) {
			if (shortestTravel > 0) {
				addBlock(
					`Travel to ${bestPoi.properties.name}`,
					currentMins,
					shortestTravel,
					'travel',
				);
			...
			addBlock(
				`Visit ${bestPoi.properties.name}`,
				currentMins,
				bestPoi.properties.typical_duration_minutes,
				'poi',
				bestPoi,
			);
			...
			if (
				nextOpenTime !== Infinity &&
				nextOpenTime - currentMins > 0 &&
				nextOpenTime - currentMins < availableMins
			) {
			...
			const categories = [
				...new Set(candidatePOIs.map((p) => p.properties.category)),
			];
			const suggestedCat = await fillGap(engine, jumpMins, categories, answers);

			addBlock(
				`Free time exploring ${suggestedCat}`,
				currentMins,
				jumpMins,
				'travel',
			);
