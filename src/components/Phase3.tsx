...
export const Phase3: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const build = async () => {
      try {
        ...
        const { blocks, routeGeoJSON } = await generateItinerary(engine, intent, answers, pois);
        setBlocks(blocks);
  ...
  return (
    <div className="loading-container animate-fade-in">
      ...
      <h2 className="form-title" style={{ marginBottom: '0.5rem' }}>Assembling Your Itinerary...</h2>
      <p style={{ color: '#9ca3af' }}>Calculating optimal paths entirely on your device</p>
    </div>
  );
};
