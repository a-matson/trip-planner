...
self.onmessage = async (event) => {
	try {
		switch (event.data.type) {
			...
			case 'search':
				search(event.data.id, event.data.query, event.data.limit ?? 10);
				break;
		}
	} catch (error) {
		postMessage({
...
async function initialise() {
	...
	const { default: locations } = await import('../data/countries+cities.json');
	...
	const totalCities = locations.reduce((total, { c }) => total + c.length, 0);
		...
		for (const city of cities) {
			...
			if (processed % 1000 === 0) {
				postMessage({
					type: 'status',
					status: 'building-index',
					progress: Math.round((processed / totalCities) * 100),
				});
			}
		}
	}
...
function search(id: number, query: string, limit: number) {
	if (!index) {
		postMessage({
			type: 'error',
